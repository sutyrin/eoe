...
- [x] Шаг 1 — базовый loop (шаг + высота + стамина).
- [x] Шаг 2 — препятствия и столкновения.
- [ ] Шаг 3 — доп. ресурсы (Warmth, O₂).
- [x] Шаг 4 — Score и базовая прогрессия.
...
- Game API: в GameState есть `stamina/maxStamina` и `altitude`; при `status: ended` доступно действие `restart`.
