...
## Devvit (E2E/GIF заметки)

- Скрины Devvit должны включать UI WebView (кнопка комментария снизу, меню сверху): снимаем клип по `shreddit-post`.
- В Devvit WebView `window.__MCP__` может быть недоступен в game frame — e2e/gif кликают реальные кнопки UI.
- Если кнопки не кликаются/не видны: проверить, что UI рендерится в `#controls` из `game.html`.
- `devvit upload` может запросить OAuth в браузере; это требует ручного подтверждения.
