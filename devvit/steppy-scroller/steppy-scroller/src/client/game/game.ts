...
export class Game extends Scene {
  ...
  resize(gameSize: Phaser.Structs.Size) {
      ...
      this.cameras.main.centerOn(
          (STEPPY_COLUMNS * CELL_SIZE) / 2, 
          (VIEW_ROWS * CELL_SIZE) / 2
      );
      this.redraw();
  }
  ...
  redraw() {
    ...
    this.updateTextObj('hud-water', width - 120, 10, `Water: ${this.state.water}`, { ...hudStyle, color: this.state.water < 5 ? '#e74c3c' : '#3498db' });
    
    if (this.state.status === 'ended') {
        this.updateTextObj('hud-gameover', width/2, height/2, 'WITHERED', {
            fontSize: '48px', color: '#c0392b', fontStyle: 'bold', stroke: '#fff', strokeThickness: 4
        }, 0.5);
    } else {
  ...
  updateTextObj(name: string, x: number, y: number, text: string, style: any, origin: number = 0) {
