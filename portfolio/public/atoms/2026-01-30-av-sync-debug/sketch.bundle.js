...
const createBaseAudioContextConstructor = (addAudioWorkletModule2, analyserNodeConstructor2, audioBufferConstructor2, audioBufferSourceNodeConstructor2, biquadFilterNodeConstructor2, channelMergerNodeConstructor2, channelSplitterNodeConstructor2, constantSourceNodeConstructor2, convolverNodeConstructor2, decodeAudioData2, delayNodeConstructor2, dynamicsCompressorNodeConstructor2, gainNodeConstructor2, iIRFilterNodeConstructor2, minimalBaseAudioContextConstructor2, oscillatorNodeConstructor2, pannerNodeConstructor2, periodicWaveConstructor2, stereoPannerNodeConstructor2, waveShaperNodeConstructor2) => {
  return class BaseAudioContext extends minimalBaseAudioContextConstructor2 {
    ...
    decodeAudioData(audioData2, successCallback, errorCallback) {
      return decodeAudioData2(this._nativeContext, audioData2).then((audioBuffer) => {
...
const createDecodeAudioData = (audioBufferStore2, cacheTestResult2, createDataCloneError2, createEncodingError2, detachedArrayBuffers, getNativeContext2, isNativeContext2, testAudioBufferCopyChannelMethodsOutOfBoundsSupport2, testPromiseSupport2, wrapAudioBufferCopyChannelMethods2, wrapAudioBufferCopyChannelMethodsOutOfBounds2) => {
  return (anyContext, audioData2) => {
    const nativeContext = isNativeContext2(anyContext) ? anyContext : getNativeContext2(anyContext);
    if (detachedArrayBuffers.has(audioData2)) {
      const err = createDataCloneError2();
      return Promise.reject(err);
    }
    try {
      detachedArrayBuffers.add(audioData2);
    } catch {
    }
    if (cacheTestResult2(testPromiseSupport2, () => testPromiseSupport2(nativeContext))) {
      return nativeContext.decodeAudioData(audioData2).then((audioBuffer) => {
        detachArrayBuffer(audioData2).catch(() => {
        });
    ...
    return new Promise((resolve, reject) => {
      const complete = async () => {
        try {
          await detachArrayBuffer(audioData2);
        } catch {
        }
      };
      const fail = (err) => {
      ...
      try {
        nativeContext.decodeAudioData(audioData2, (audioBuffer) => {
          if (typeof audioBuffer.copyFromChannel !== "function") {
            wrapAudioBufferCopyChannelMethods2(audioBuffer);
            wrapAudioBufferGetChannelDataMethod(audioBuffer);
          }
...
class Context extends BaseContext {
  ...
  decodeAudioData(audioData2) {
    return this._context.decodeAudioData(audioData2);
  }
  /**
...
function createSequence(synth2, seqConfig) {
  ...
  const sequence2 = new Sequence((time, note) => {
...
function initAudio(config2) {
  ...
  synth$1 = createSynth(config2.synth || {});
  synth$1.connect(effectsChain.chain);
  audioDataProvider = new AudioDataProvider(effectsChain.output, {
    fftSize: config2.analysis?.fftSize || 512,
    smoothing: config2.analysis?.smoothing || 0.3,
    outputSmoothing: config2.analysis?.outputSmoothing || 0.1,
    beatThreshold: config2.analysis?.beatThreshold || 0.3
  });
  sequence$1 = createSequence(synth$1, config2.sequence || {});
}
async function startAudio(bpm = 60) {
...
const synth = { "type": "membrane", "pitchDecay": 0.05, "octaves": 3, "envelope": { "attack": 1e-3, "decay": 0.1, "sustain": 0, "release": 0.1 } };
const sequence = { "notes": ["C2", "C2", "C2", "C2", "C2", "C2", "C2", "C2", "C2", "C2"], "duration": "16n", "interval": "2n" };
const transport = { "bpm": 60 };
const analysis = { "fftSize": 512, "smoothing": 0.3, "outputSmoothing": 0.1, "beatThreshold": 0.3 };
const config = {
  synth,
  sequence,
  transport,
...
function draw(p) {
  ...
  p.text(`Time: ${(p.millis() / 1e3).toFixed(1)}s`, 10, 50);
  p.text(`Playing: ${getIsPlaying()}`, 10, 70);
}
async function onPlayClick(p) {
  await startAudio(config.transport.bpm);
}
function onStopClick(p) {
  stopAudio();
