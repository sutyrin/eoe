...
В отличие от Claude Code, Codex после `resume` не гарантирует полный контекст. Поэтому Codex лучше не перезапускать,
пока важные решения/правки не выгружены в файлы (например, `AGENTS.md` или `docs/*`). Это неудобно, но нужно учитывать.
