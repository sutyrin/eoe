...
- глубоко проанализировать ответы на вопросы и продолжить анализировать и доплонять agents. 
- спроектировать конкретную структуру разработки в соответствии с требованиями к mcp game api и пирамиде тестов.
- зафиксировать текущий стек (js/ts, vite, phaser, сборщик, выкладка на vercel и т.п.) и расписать инструкции по работе для агента.
...
## TODO (MVP для отладки MCP-подхода)

Цель — быстро доказать жизнеспособность MCP в браузере + тесты через Playwright на примере Steppy Scroller.

1) Создать общий модуль `/packages/game-api` с:
   - типами `GameState`, `Action`;
   - адаптером для `window.__MCP__` (register/getState/getActions/act);
   - минимальным dev-логгером (`console.log` c префиксом).
2) В `games/steppy-scroller`:
   - подключить `game-api` и зарегистрировать начальное состояние;
   - реализовать 2–3 действия (например, `step-left`, `step-right`, `step-up`);
   - синхронизировать UI с `state.actions`.
3) Написать минимум тестов:
   - unit: редьюсер состояния,
   - api: smoke (getState + 1–2 act),
   - e2e: отрисовка UI + клик действия + сверка state через `window.__MCP__`.
4) Минимальный deploy на Vercel и smoke-проверка через Playwright.
