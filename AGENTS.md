...
Базовый API живет в общем слое репозитория и доступен каждой игре из подкаталога. Это минимальный набор, который
покрывает текущие требования к тестам и итерациям разработки.

- `GET /mcp/state` — текущее состояние игры, всегда включает:
...
- `GET /mcp/actions` — список доступных действий (может совпадать с `state.actions`).
- `POST /mcp/act` — выполнить действие `{ actionId, payload? }`, возвращает обновленный state.
...
Это минимальный «канон». Он может расширяться, но новые поля и эндпоинты должны быть обратно совместимы
