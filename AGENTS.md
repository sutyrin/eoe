...
- [x] разработать и дописать в этот файл общие принципы шаблона игры (см. секцию "Шаблон игры: принципы и идеи").
...
Этот раздел описывает концептуальную основу для серии игр в этом репозитории.
...
2.  **Управление (Mobile-first):**
    ...
    *   Количество кнопок: Минималистичное, обычно 2 или 3 (например: "Влево", "Вперед", "Вправо" или "Атака", "Защита").
...
4.  **Визуальный стиль:**
    *   Живая картинка: Несмотря на пошаговость, мир не статичен. Используются idle-анимации героя, врагов, фоновые эффекты (ветер, партиклы), чтобы игра чувствовалась "живой".
    *   Четкая сетка (Grid): Игровое поле дискретно, что упрощает визуальное считывание и навигацию агента.
...
1.  **"Бесконечный выбор пути" (Branching Paths):**
    *   Классический раннер, но пошаговый. На пути встречаются препятствия (камни, ямы). Игрок должен выбрать полосу движения (лево/центр/право), чтобы избежать урона и собрать монетки.
    *   *Пример:* Steppy Scroller.

2.  **"Тактическое восхождение" (Resource Management):**
    *   Каждый шаг тратит ресурс (выносливость/топливо). На поле разбросаны восполняющие предметы и враги.
    *   Игрок выбирает: пойти коротким, но опасным путем (через врага) или длинным, но безопасным (тратя больше ресурса).

3.  **"Камень-Ножницы-Бумага" (Encounter Combat):**
    *   Путь блокируют враги разных типов (Красный, Зеленый, Синий).
    *   У игрока есть 3 кнопки-стойки. Нужно выбрать правильную стойку *перед* шагом на клетку с врагом, чтобы победить его без урона.

4.  **"Сокобан-лайт" (Pushing):**
    *   На пути встречаются блоки, которые можно толкать перед собой.
    *   Задача: толкать блоки так, чтобы заполнять ямы впереди или активировать кнопки, открывающие проход дальше.

5.  **"Сбор комбинаций" (Match-3 path):**
    *   Игрок движется по полю, заполненному цветными тайлами.
    *   Задача: наступать подряд на 3 тайла одного цвета, чтобы получить бонус или разрушить стену впереди. Прерывание серии штрафуется.
...
2.  **Cyberpunk Hacker (Взлом сервера):**
    ...
    *   Препятствия: Файрволы, вирусы.
    *   Бонусы: Пакеты данных, ключи шифрования.
    *   Стиль: Неон, глитч-эффекты, матрица кода.
...
3.  **Infinite Garden (Волшебный боб):**
    ...
    *   Атмосфера: Медитативная, дзен, звуки природы.
...
4.  **Space Elevator (Космический лифт):**
    ...
    *   Фон меняется от голубого неба до черного космоса со звездами.
